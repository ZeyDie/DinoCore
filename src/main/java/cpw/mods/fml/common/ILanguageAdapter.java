...
public interface ILanguageAdapter {
    ...
    public static class ScalaAdapter implements ILanguageAdapter {
        ...
        {
            Class<?> proxyTarget1 = proxyTarget;
            try
            {
                ...
                if (!proxyTarget1.getName().endsWith("$"))
                {
                    // Get internal class generated by Scala.
                    proxyTarget1 = Class.forName(proxyTarget1.getName() + "$", true, proxyTarget1.getClassLoader());
                }
            }
            catch (final ClassNotFoundException e)
            {
                // Not a singleton, look for @Instance field as a fallback.
                FMLLog.log(Level.INFO, e, "An error occured trying to load a proxy into %s.%s. Did you declare your mod as 'class' instead of 'object'?", proxyTarget1.getSimpleName(), target.getName());
                return;
            }
            ...
            // singletons.
            final Object targetInstance = proxyTarget1.getField("MODULE$").get(null);
            ...
            try
            {
                ...
                for (final Method setter : proxyTarget1.getMethods())
                {
            ...
            {
                FMLLog.log(Level.SEVERE, e, "An error occured trying to load a proxy into %s.%s", proxyTarget1.getSimpleName(), target.getName());
                throw new LoaderException(e);
            }

            // If we come here we could not find a setter for this proxy.
            FMLLog.severe("Failed loading proxy into %s.%s, could not find setter function. Did you declare the field with 'val' instead of 'var'?", proxyTarget1.getSimpleName(), target.getName());
    ...
    public static class JavaAdapter implements ILanguageAdapter {
        ...
        @Override
        public void setProxy(final Field target, final Class<?> proxyTarget, final Object proxy) throws IllegalArgumentException, IllegalAccessException, NoSuchFieldException,
                SecurityException
        {
