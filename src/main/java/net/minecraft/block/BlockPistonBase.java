...
public class BlockPistonBase extends Block
{
    ...
    {
        final int k = getOrientation(par2);
    ...
    @SideOnly(Side.CLIENT)
    public static Icon getPistonBaseIcon(final String par0Str)
    {
    ...
    {
        final int l = determineOrientation(par1World, par2, par3, par4, par5EntityLivingBase);
    ...
    private void updatePistonState(final World par1World, final int par2, final int par3, final int par4)
    {
        ...
        final int i1 = getOrientation(l);

        if (i1 != 7)
        {
            final boolean flag = this.isIndirectlyPowered(par1World, par2, par3, par4, i1);

            if (flag && !isExtended(l))
            {
                // CraftBukkit start
                final int length = canExtend_IntCB(par1World, par2, par3, par4, i1); // Cauldron - update from rename

                if (length >= 0)
                {
                    final org.bukkit.block.Block block = par1World.getWorld().getBlockAt(par2, par3, par4);
                    final BlockPistonExtendEvent event = new BlockPistonExtendEvent(block, length, CraftBlock.notchToBlockFace(i1));
            ...
            else if (!flag && isExtended(l))
            {
                // CraftBukkit start
                final org.bukkit.block.Block block = par1World.getWorld().getBlockAt(par2, par3, par4);
                final BlockPistonRetractEvent event = new BlockPistonRetractEvent(block, CraftBlock.notchToBlockFace(i1));
                par1World.getServer().getPluginManager().callEvent(event);
    ...
    public boolean onBlockEventReceived(final World par1World, int par2, int par3, int par4, final int par5, final int par6)
    {
        ...
        if (!par1World.isRemote)
        {
            final boolean flag = this.isIndirectlyPowered(par1World, par21, par31, par41, par6);

            if (flag && par5 == 1)
            {
                par1World.setBlockMetadataWithNotify(par21, par31, par41, par6 | 8, 2);
                return false;
            }
        ...
        if (par5 == 0)
        {
            if (!this.tryExtend(par1World, par21, par31, par41, par6))
            {
                return false;
            }

            par1World.setBlockMetadataWithNotify(par21, par31, par41, par6 | 8, 2);
            par1World.playSoundEffect((double) par21 + 0.5D, (double) par31 + 0.5D, (double) par41 + 0.5D, "tile.piston.out", 0.5F, par1World.rand.nextFloat() * 0.25F + 0.6F);
        }
        else if (par5 == 1)
        {
            final TileEntity tileentity = par1World.getBlockTileEntity(par21 + Facing.offsetsXForSide[par6], par31 + Facing.offsetsYForSide[par6], par41 + Facing.offsetsZForSide[par6]);
            ...
            par1World.setBlock(par21, par31, par41, Block.pistonMoving.blockID, par6, 3);
            par1World.setBlockTileEntity(par21, par31, par41, BlockPistonMoving.getTileEntity(this.blockID, par6, par6, false, true));

            if (this.isSticky)
            {
                final int j1 = par21 + Facing.offsetsXForSide[par6] * 2;
                final int k1 = par31 + Facing.offsetsYForSide[par6] * 2;
                final int l1 = par41 + Facing.offsetsZForSide[par6] * 2;
                ...
                {
                    final TileEntity tileentity1 = par1World.getBlockTileEntity(j1, k1, l1);

                    if (tileentity1 instanceof TileEntityPiston)
                    {
                        final TileEntityPiston tileentitypiston = (TileEntityPiston)tileentity1;
                ...
                {
                    par21 += Facing.offsetsXForSide[par6];
                    par31 += Facing.offsetsYForSide[par6];
                    par41 += Facing.offsetsZForSide[par6];
                    par1World.setBlock(par21, par31, par41, Block.pistonMoving.blockID, j2, 3);
                    par1World.setBlockTileEntity(par21, par31, par41, BlockPistonMoving.getTileEntity(i2, j2, par6, false, false));
                    par1World.setBlockToAir(j1, k1, l1);
                }
                else if (!flag1)
                {
                    par1World.setBlockToAir(par21 + Facing.offsetsXForSide[par6], par31 + Facing.offsetsYForSide[par6], par41 + Facing.offsetsZForSide[par6]);
                }
            }
            else
            {
            ...
            par1World.playSoundEffect((double) par21 + 0.5D, (double) par31 + 0.5D, (double) par41 + 0.5D, "tile.piston.in", 0.5F, par1World.rand.nextFloat() * 0.15F + 0.6F);
        }
     ...
     */
    public static int determineOrientation(final World par0World, final int par1, final int par2, final int par3, final EntityLivingBase par4EntityLivingBase)
    {
        ...
        {
            final double d0 = par4EntityLivingBase.posY + 1.82D - (double)par4EntityLivingBase.yOffset;
        ...
        final int l = MathHelper.floor_double((double)(par4EntityLivingBase.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3;
    ...
    private static boolean canExtend(final World world, final int i, final int j, final int k, final int l) {
     ...
     */
    private static int canExtend_IntCB(final World par0World, final int par1, final int par2, final int par3, final int par4) // Cauldron - rename and wrap
    {
        ...
        while (true)
        {
            if (l1 < 13)
            {
                ...
                final int i2 = par0World.getBlockId(i1, j1, k1);
        ...
        while (true)
        {
            ...
            if (l1 < 13)
            {
                ...
                if (!par1World.isAirBlock(i1, j1, k1))
                {
                    ...
                    final float chance = (Block.blocksList[i2] instanceof BlockSnow ? -1.0f : 1.0f);
            ...
            for (aint = new int[13]; i1 != par2 || j1 != par3 || k1 != par4; k1 = j3)
            {
                ...
                final int k3 = par1World.getBlockId(l2, i3, j3);
                final int l3 = par1World.getBlockMetadata(l2, i3, j3);
