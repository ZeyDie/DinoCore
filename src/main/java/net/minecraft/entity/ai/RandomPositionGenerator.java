...
public class RandomPositionGenerator
{
    ...
    private static Vec3 findRandomTargetBlock(final EntityCreature par0EntityCreature, final int par1, final int par2, final Vec3 par3Vec3)
    {
        ...
        {
            final double d0 = (double)(par0EntityCreature.getHomePosition().getDistanceSquared(MathHelper.floor_double(par0EntityCreature.posX), MathHelper.floor_double(par0EntityCreature.posY), MathHelper.floor_double(par0EntityCreature.posZ)) + 4.0F);
            final double d1 = (double)(par0EntityCreature.func_110174_bM() + (float)par1);
            flag1 = d0 < d1 * d1;
        }
        else
        {
        ...
        for (int j1 = 0; j1 < 10; ++j1)
        {
            ...
            if (par3Vec3 == null || (double)k1 * par3Vec3.xCoord + (double)i2 * par3Vec3.zCoord >= 0.0D)
            {
                ...
                {
                    final float f1 = par0EntityCreature.getBlockPathWeight(k1, l1, i2);
